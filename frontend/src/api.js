...
async function request(path, { method = 'GET', body, auth = true, _retried = false } = {}) {
  ...
  try {
    ...
    throw new Error(
      'Cannot reach KabQue API. Make sure Django is running on port 8001 and the frontend is using the Vite proxy.'
    );
  }
    ...
    const err = new Error(
      errorFromPayload(
        ...
        res.status === 502 || res.status === 503 || res.status === 504
          ? 'KabQue API is unavailable. Start Django on port 8001 and try again.'
          : 'Request failed'
      )
    );
